...
export class CustomReuseStrategy implements RouteReuseStrategy {
  ...
  store(route: ActivatedRouteSnapshot, handle: DetachedRouteHandle): void {
    ...
    // routes are stored by path - the key is the path name, and the handle is stored under it
    // so that you can only ever have one object stored for a single path
    this.storedRoutes[route.routeConfig.path] = storedRoute;
  }

  /**
   * Determines whether or not there is a stored route and, if there is,
   * whether or not it should be rendered in place of requested route
  ...
  shouldAttach(route: ActivatedRouteSnapshot): boolean {
    ...
    // this decides whether the route already stored should be rendered in place of the requested route,
    // and is the return value at this point we already know that the paths match because the storedResults
    // key is the route.routeConfig.path so, if the route.params and route.queryParams also match,
    // then we should reuse the component
    if (canAttach) {
      ...
      console.log(
        'deciding to attach...', route,
        'does it match?', this.storedRoutes[route.routeConfig.path].snapshot,
  ...
  shouldReuseRoute(future: ActivatedRouteSnapshot, curr: ActivatedRouteSnapshot): boolean {
    console.log(
      'deciding to reuse',
      'future', future.routeConfig,
      'current', curr.routeConfig,
